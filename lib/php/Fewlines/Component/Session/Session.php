...
use Fewlines\Component\Session\Cookie\Cookie as NativeCookie;
use Fewlines\Component\Session\Cookie\Session as NativeSession;
use Fewlines\Component\Session\Result;
...
class Session
{
     ...
     * @var string
     */
    const COOKIE_NAMESPACE = 'Fewlines\Component\Session\Cookie\Cookie';
     ...
     * @var string
     */
    const SESSION_NAMESPACE = 'Fewlines\Component\Session\Cookie\Session';
    ...
    private function createCookie($name, $content, $lifetime, $encrypt, $path) {
        ...
        $cookie = new NativeCookie;
        $cookie->setName($name)->setContent($content)->setLifetime($lifetime)->setPath($path)->setEncrypted($encrypt)->create();
    ...
    private function createSession($name, $content) {
        if (false == self::$isStarted) {
            throw new Exception\SessionNotStartedException("Please start the session first (before any output)");
        }

        $name = self::convertName($name);

        $session = new NativeSession;
        $session->setName($name)->setContent($content)->create();
